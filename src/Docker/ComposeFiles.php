...
class ComposeFiles
{
    ...
    /**
     * The full compose stack ngramx/docker compose actually applies: base file
     * plus any layered overrides that exist on disk.
     *
     * @return list<string>
     */
    public static function allInputFiles(string $composeFile): array
    {
        return array_merge([$composeFile], self::layeredFiles($composeFile));
    }

    public static function isGeneratedOverride(string $path): bool
    {
        if (basename($path) !== self::OVERRIDE_FILE || !is_file($path)) {
            return false;
        }

        $content = file_get_contents($path);

        return is_string($content) && str_contains($content, 'Generated by Ngramx CLI');
    }
}
