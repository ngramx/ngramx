...
- Prebuilt `max_age_hours` now surfaces real source errors (missing dump, S3 resolve/HEAD/network failures) instead of collapsing them into “Could not determine Prebuilt artifact age”. `lastModified()` returns null only when the artifact exists but its age is unknown.
- Namespaced network matching no longer falls back to any `_{logical}` suffix after a `{project}_{logical}` miss, so a clone cannot attach to another stack's network after `-p` stops this project's `db`.
